...
oaz.start = function(config) {
    ...
    db.open(function(err) {
        ...
        var matchSpotify = function(from, to, message, matches) {
            var id = matches[1];
            var url = 'http://ws.spotify.com/lookup/1/.json?uri=' + id;
        ...
        client.match(/(http:\/\/open\.spotify\.com\/[\w\/]+)/g, matchSpotify);

        // Spotify URI
        client.match(/spotify:(\w+):(\w+)/g, function(from, to, message, matches) {
            var type = matches[1];
            var id = matches[2];
            var url = 'http://open.spotify.com/' + type + '/'+ id;

            client.say(to, from + ': That\'s probably supposed to be ' + url);

            matchSpotify(from, to, message, [message, url]);
        });
        ...
        client.match(/wimp\.no\/(artist|album|track)\/\d+/g, function(from, to, message, matches) {
            ...
            request(url, function(err, response, body) {
                if (response.statusCode == 200) {
                    ...
                    if (titleMatches) {
                        client.say(to, ('WiMP ' + type.substr(0, 1).toUpperCase() + type.substr(1)).bold + ': ' + titleMatches[0]);
                    }
                }
            });
        });
...
if (!module.parent) {
    var config = oaz.getConfig();
    config.debug = true;
    config.channels.push('#sof_priv dead');
