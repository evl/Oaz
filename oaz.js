                        ...
                        if (link) {
                            ...
                            if (!entry.processed) {
                                ...
                                db.set(key, link, function(err) {
                                    ...
                                    var result = '{0}: {1}'.format(entry.user, age.toBold())
        ...
        */
        ...
        bot.watch_for(/youtube.*v=([^#&\s]+)/, function(message) {
            ...
            request(options, function(err, response, body) {
                ...
                if (entry) {
                    message.say('{0}: {1}'.format('YouTube'.toBold(), entry.title.$t));
                }
            });
        });
        ...
        bot.watch_for(/vimeo\.com\/(\d+)/, function(message) {
            ...
            request(options, function(err, response, body) {
                ...
                if (data) {
                    message.say('{0}: {1}'.format('Vimeo'.toBold(), data[0].title));
                }
            });
        });
        ...
        bot.watch_for(/(http:\/\/open\.spotify\.com\/[\w\/]+)/, function(message) {
            ...
            request(options, function(err, response, body) {
                if (response.statusCode == 200) {
                    ...
                    message.say('{0}: {1}'.format(('Spotify ' + type).toBold(), title));
                }
            });
        });
        ...
        bot.watch_for(/wimp\.no\/(artist|album|track)\/\d+/, function(message) {
            ...
            request(options, function(err, response, body) {
                if (response.statusCode == 200) {
                    ...
                    if (matches) {
                        var title = matches[1];
                        message.say('{0}: {1}'.format(('WiMP ' + type.capitalize()).toBold(), title));
                    }
                }
            });
        });
        ...
        bot.watch_for(/^!(?:weather|temp|termo|temperature) (.+)/, function(message) {
            ...
            request(options, function(err, response, body) {
                ...
                if (description) {
                    message.say('{0}: {1}°C {2}, {3}% Humidity'.format(city[1].toBold(), temperature[1], description[1], humidity[1]));
                }
            });
        });
    }).connect(options);
